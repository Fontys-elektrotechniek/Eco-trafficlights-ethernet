...
#define HOST_TO_PING	"192.168.50.1"	// Address that ICMP client will ping.  If the DNS client module is not available in the stack, then this hostname is ignored and the local gateway IP address will be pinged instead.
...
#if defined(STACK_USE_UART)
            memset(UARTText, ' ', 32);
            uitoa((WORD) TickConvertToMilliseconds((DWORD) ret), &UARTText[16 + 7]);
            putrsUART("Replay: ");
            putrsUART(UARTText);
            putrsUART("ms\r\n");
#endif
