...
static HTTP_IO_RESULT HTTPPostLCD(void)
{
	...
	switch(curHTTP.smPost)
	{
		...
		#elif defined(STACK_USE_UART)
			// Copy up to 32 characters to the LCD
			if(strlen((char*)cDest) < 32u)
			{
				memset(UARTText, ' ', 32);
				strcpy((char*)UARTText, (char*)cDest);
			}
			else
			{
				memcpy(UARTText, (void *)cDest, 32);
			}
			putrsUART(UARTText);
			putrsUART((ROM char*)"\r\n");
		#endif	
...
{
	TCPPutROMString(sktHTTP, (ROM void*)TRAFFICLIGHTS_VERSION);
}
...
void HTTPPrint_lcdtext(void)
{
	...
	while(len > 12u && curHTTP.callbackPos)
	{
		...
		#elif defined(STACK_USE_UART)
		if(UARTText[32-curHTTP.callbackPos] == ' ' || UARTText[32-curHTTP.callbackPos] == '\0')
			len -= TCPPutROMArray(sktHTTP, (ROM BYTE*)"&nbsp;", 6);
		else
			len -= TCPPut(sktHTTP, UARTText[32-curHTTP.callbackPos]);
		#endif
...
void HTTPPrint_hellomsg(void)
{
	...
	if(ptr != NULL)
	{
		TCPPutROMString(sktHTTP, (ROM BYTE*)"Hallo, ");
...
void HTTPPrint_cookievar(void)
{
	BYTE *ptr;
	
	ptr = HTTPGetROMArg(curHTTP.data, (ROM BYTE*)"fav");
