...
static inline void OpenSPIChn(unsigned int config1, unsigned int config2)
{
...
static inline void CloseSPIChn(void)
{
#if defined (TCP_USE_SPI_1)
    CloseSPI1();
#else
...
static inline void putcSPIChn(unsigned int data_out)
{
#if defined (TCP_USE_SPI_1)
    putcSPI1(data_out);
#else
...
static inline unsigned int getcSPIChn(void)
{
#if defined (TCP_USE_SPI_1)
    return getcSPI1();
#else
...
int MACInit(int waitComplete)
{
    ...
    OpenSPIChn(SPI_OPEN_CFG_1, SPI_OPEN_CFG_2);
...
NET_PKT* pPktBfr;
    ...
    if((ReadETHReg(ECON1).ECON1bits.TXRTS) && (_txCollisionEnable))
    {
        ...
        if(collision_ticks > TX_COLLISION_TIMEOUT)
        {
...
BYTE ByteTransfer( BYTE SendByte )
{
    ...
    putcSPIChn((unsigned int) SendByte);
