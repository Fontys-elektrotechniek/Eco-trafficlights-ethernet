...
int         TCPXmitPkt(DWORD remoteIP, WORD localPort, WORD remotePort, DWORD tseq, DWORD tack, BYTE flags);
int         TCPXmitSktPkt(struct MCSocket* pSock, unsigned int flags, NET_PKT* sbfr, WORD DataLen);

BOOL		MAC_IsPktqueued( NET_PKT* sbfr );
static WORD TCPAddPseudoChecksum(DWORD SrcIP, DWORD DestIP, DWORD iniSum, WORD tcpLen);
...
void TCPInitiateCnxn( struct MCSocket* pSock )
{
    ...
    TCPXmitSktPkt(pSock, SYN, NULL, 0);
    pSock->TcpSEQ++, //SYN takes 1 seq number
    pSock->smState = TCP_SYN_SENT;
}

/*********************************************************************
 * Function:        WORD TCPAddPseudoChecksum( DWORD SrcIP, DWORD DsetIP, DWORD iniSum, WORD tcpLen )
 *
 * PreCondition:    None
 * Input:			SrcIP - local IP address
 *					DestIP - destination IP address
 *					iniSum - initial value of the checksun
 *					tcpLen - data length
 * Output:          checksum for the packet
 * Side Effects:    None
 * Overview:        Adds the pseudo header for the TCP checksum calculation.
 * Note:
 ********************************************************************/
static WORD TCPAddPseudoChecksum(DWORD SrcIP, DWORD DestIP, DWORD iniSum, WORD tcpLen)
{
...
/*********************************************************************
 * Function:        int TCPXmitSktPkt((struct MCSocket* pSock, unsigned int flags, NET_PKT* sbfr, WORD DataLen)
 *
 * PreCondition:    None
 * 
 * Input:			pSock 		- pointer to socket structure
 ...
 *
 * Note:            sbfr == 0 specifies that no socket data is transmitted.
 *                  It's rather an acknowledge/special packet of some sort.
 *                  A new NET_PKT will be allocated and will be transmitted.
 ********************************************************************/
int TCPXmitSktPkt(struct MCSocket* pSock, unsigned int flags, NET_PKT* sbfr, WORD DataLen)
{
    TCP_HEADER*     pHdr;
    ...
    hLen = sizeof(TCP_HEADER);

    tseq=pSock->TcpSEQ;
    if(flags&KEEP_ALIVE_PKT)
    {
        tseq-=1;
    }
    
    if ( flags & SYN )
    {   // SYN packet shouldn't carry any data
	...
	IP_FormatPacket( sbfr, pSock->remoteIP, IP_PROT_TCP, TcpLen );
	
	pHdr = 	(TCP_HEADER*)sbfr->pTransportLayer;

    if((flags&USE_PARENT_PORT) && pSock->pParentSocket )
    {
        pHdr->SourcePort = pSock->pParentSocket->localPort;
    }
    else
    {
        pHdr->SourcePort = pSock->localPort;
    }


    pHdr->DestPort      = pSock->remotePort;
    
    _dwordToArray(tseq, pHdr->SeqNum);
    _dwordToArray(pSock->TcpACK, pHdr->AckNum);

    pHdr->Flags			= (BYTE)flags;
    pHdr->Window 		= pSock->RxBfrSize;
	...
	pHdr->HdrLen = (BYTE)(hLen << 2);

    // calculate the checksum
    tcpChkSum=0;
#ifdef _STACK_USE_ZC
	if(socketHasData && pSock->sockFlags&SOCK_FLAG_ZCOPY)
	{	// split packet
        // add the TCP header
		checksum16_partial((BYTE*)pHdr, sizeof(TCP_HEADER), &tcpChkSum);
		// add the payload
		checksum16_partial(sbfr->PktDcpt[1].bDcpt.pBuff, DataLen, &tcpChkSum);
        pHdr->Checksum =~TCPAddPseudoChecksum(SysIPAddr.Val, pSock->remoteIP, tcpChkSum, TcpLen);
	}
	else
	{
		checksum16_partial(sbfr->pTransportLayer, TcpLen, &tcpChkSum);
		pHdr->Checksum = ~TCPAddPseudoChecksum( SysIPAddr.Val, pSock->remoteIP, tcpChkSum, TcpLen );
	}		
#else
    checksum16_partial(sbfr->pTransportLayer, TcpLen, &tcpChkSum);
 ...
 *					DWORD tseq,	DWORD tack, BYTE flags)
 *
 * PreCondition:    None
 ...
 *					flags 		- transmit flags
 *
 * Output:          0 if successful,  SOCKET_BFR_ALLOC_ERROR else
 *  
 * Side Effects:    None
 * 
 * Overview:        Package and transmit a NO_LOAD TCP packet!
 *                  This is a packet that carries no TCP load.
 *                  It cannot be an SYN packet either.
 * 
 * Note:            Similar to TCPXmitSktPkt but when the packet was not initiated by an existent socket.
 ********************************************************************/
int TCPXmitPkt(DWORD remoteIP, WORD localPort, WORD remotePort, DWORD tseq, DWORD tack, BYTE flags)
{
    NET_PKT*        sbfr;
    ...
    pHdr->Flags			= flags;
    pHdr->Window 		= TCP_DEFAULT_RX_BFR_SZ;
    ...
    tcpChkSum=0;
    checksum16_partial(sbfr->pTransportLayer, TcpLen, &tcpChkSum);
    pHdr->Checksum =~TCPAddPseudoChecksum(SysIPAddr.Val, remoteIP, tcpChkSum, TcpLen);
...
void TCP_RxHandler( NET_PKT *RxPkt )
{
    ...
    // we can use the hardware engine RX check
    tcpChkSum=RxIPLoadChecksum(RxPkt);
#else        
    tcpChkSum=0;
	checksum16_partial(RxPkt->pTransportLayer, RxPkt->PktLen, &tcpChkSum);
#endif
 
    if ( TCPAddPseudoChecksum( src_ip, SysIPAddr.Val, tcpChkSum, RxPkt->PktLen ) != 0xFFFF )
    {
...
int TCPTransmitData( struct MCSocket* pSock, BYTE* loadBuff, int len )
{
	...
	if( (pSock->sockFlags&(TCP_FIRST_SEND|TCP_NO_DELAY|SOCK_FLAG_ZCOPY)) || pSock->TxDataIndex == pSock->TxBfrSize )
	{
        ...
        TCPXmitSktPkt(pSock, PSH|ACK|USE_PARENT_PORT, pSock->pTxActivePacket, BfrSize);
...
void ProcessSYN( struct MCSocket* pListenSock, NET_PKT* pNetPkt )
{
	...
	TCPXmitSktPkt(pNewSock, SYN|ACK|USE_PARENT_PORT, NULL, 0);
    pNewSock->TcpSEQ++; // SYN takes up 1 seq#
...
{	
	...
	{
		TCPXmitSktPkt(pSocket, TxFlags|USE_PARENT_PORT, pSocket->pTxActivePacket, pSocket->TxDataIndex);
...
void TCPCloseSocket( struct MCSocket *pSock )
{
	...
	if( TxFlags )
	{
		TCPXmitSktPkt(pSock, TxFlags|USE_PARENT_PORT, NULL, 0);
...
void HandleTCPSegment( SOCKET s, NET_PKT* pNetPkt, WORD len )
{
	...
	case TCP_LISTEN:
		...
		if( !lp )
		{
			...
			TCPXmitPkt(src_ip, pTCP->DestPort, pTCP->SourcePort, seq_num, ack_num, RST|ACK);
		}	
	...
	case TCP_SYN_RCVD:
		...
		if( len )
		{
			TCPXmitSktPkt(pSocket, ACK|USE_PARENT_PORT, NULL, 0);
		}
	...
	case TCP_SYN_SENT:
        ...
        if ( pTCP->Flags & SYN ) //Connection Request reply from remote client
        {
			...
			TCPXmitSktPkt(pSocket, ACK, NULL, 0);
	...
	case TCP_LAST_ACK: //waiting for ack on our passive FIN reply
		...
		if( TxFlags )
		{			
			TCPXmitSktPkt(pSocket, TxFlags|USE_PARENT_PORT, NULL, 0);
	    }
    ...
    for ( s = 0; s < _StackSockets; s++,pSocket++ )
    {
		...
		if( pSocket->smState == TCP_EST )
		{
			...
			if( diffTicks > TCP_KEEP_ALIVE_TIMEOUT_VAL )		
			{
				...
				{						
					TCPXmitSktPkt(pSocket, PSH|ACK|KEEP_ALIVE_PKT|USE_PARENT_PORT, NULL, 0);
	    ...
	    if( pSocket->smState == TCP_SYN_SENT )
	    {
		    ...
		    TCPXmitSktPkt(pSocket, SYN, NULL, 0);
