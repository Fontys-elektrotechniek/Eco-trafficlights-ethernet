...
void UDP_RxHandler( NET_PKT *RxPkt )
{
    ...
    {   // transmitter generated a checksum
        ...
        udpChkSum=RxIPLoadChecksum(RxPkt);
    #else
        // calculate software packet checksum
     	udpChkSum=checksum16((BYTE*)pUDP, swaps(pUDP->Length));
    #endif
        ...
        vhdr.proto 	= IP_PROT_UDP;
        vhdr.len	= pUDP->Length;
        checksum16_partial((BYTE*)&vhdr, sizeof(struct tempHdr), &udpChkSum );
        udpChkSum= (WORD)~fold_checksum( udpChkSum );
