...
{
    volatile unsigned long flags;        // Current state flags.

    volatile void         *rx_buff;      // Rx data pointer

    volatile BYTE          rx_size;      // Number of bytes received.

    LINE_CODING            line_coding;  // Buffer to store line coding information

    volatile void         *tx_buff;      // Tx Buffer pointer.

    volatile BYTE          tx_size;      // Tx Data size.
